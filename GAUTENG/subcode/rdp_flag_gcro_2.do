...
local qry = "
   ...
   JOIN gcro AS G ON C.cluster = G.cluster 
	";
...
odbc exec("CREATE TABLE placebo_cluster AS
         SELECT G.* FROM gcro_conhulls AS G 
         LEFT JOIN rdp_cluster AS R ON R.cluster = G.cluster
         LEFT JOIN rdp_cluster_2001 AS R1 ON R1.cluster = G.cluster
         JOIN gcro AS GC ON G.cluster = GC.cluster 
         WHERE R.mode_yr_rdp IS NULL AND R1.mode_yr_rdp IS NULL            AND GC.area>=.5;
         "), dsn("gauteng");
...
* tab rdp_max  if c_n==1  ; 
* bys buyer_name: g bN=_N   ;
* bys seller_name: g sN=_N    ;
* bys purch_price: g pN =_N ;

*** ADD IN prop devs like   YIK HO SANLI PROP DEVELOPMENT  ;
* egen rdp_max = max(rdp), by(cluster) ; 
* cap drop c_id
* bys purch_price cluster seller_name rdp: g c_id = _N
* tab seller_name  if rdp==0 & purch_price<20000 & c_id>20
* replace rdp=1 if rdp==0 & purch_price<20000 & c_id>20
* cap drop rdp_max
* egen rdp_max = max(rdp), by(cluster) 
* tab rdp_max  if c_n==1   ;
