...
global ddd_regs_d = 1; /* Done! */
global ddd_regs_t = 0; 
global ddd_table  = 1; /* Done! */
...
foreach level in `r(levels)' {;
    ...
    gen dists_all_`level'_het  = (dists_joined == `level') & het==1; 
    gen dists_rdp_`level'_het  = (dists_joined == `level' & placebo==0)  & het==1;
    gen dists_post_`level'_het = (dists_joined == `level' & prepost_reg_joined ==1) & het==1;
    gen dists_rdp_post_`level'_het = (dists_joined == `level' & placebo==0 & prepost_reg_joined==1) & het==1;
    gen dists_other_`level'_het = (dists_joined == `level' & prepost_reg_joined ==2) & het==1;
    gen dists_rdp_other_`level'_het = (dists_joined == `level' & placebo==0 & prepost_reg_joined ==2) & het==1;
    
    gen dists_all_`level'  = (dists_joined == `level') & het==0; 
    gen dists_rdp_`level'  = (dists_joined == `level' & placebo==0)  & het==0;
    gen dists_post_`level' = (dists_joined == `level' & prepost_reg_joined ==1)  & het==0;
    gen dists_rdp_post_`level' = (dists_joined == `level' & placebo==0 & prepost_reg_joined==1)  & het==0;
    gen dists_other_`level' = (dists_joined == `level' & prepost_reg_joined ==2)  & het==0;
    gen dists_rdp_other_`level' = (dists_joined == `level' & placebo==0 & prepost_reg_joined ==2)  & het==0;

    global dists_all "
      dists_all_`level'_het dists_rdp_`level'_het
      dists_post_`level'_het dists_rdp_post_`level'_het
      dists_other_`level'_het dists_rdp_other_`level'_het
...
omit dists_all 
  dists_all_${max}_het dists_rdp_${max}_het
  dists_post_${max}_het dists_rdp_post_${max}_het
  dists_other_${max}_het dists_rdp_other_${max}_het
  dists_all_$max dists_rdp_$max
  dists_post_$max dists_rdp_post_$max
  dists_other_$max dists_rdp_other_$max ;

gen rdp_het = placebo==0 & het==1; 
gen post_het = (prepost_reg_joined ==1 ) & het==1; 
gen rdppost_het = rdp_het*post_het ; 
gen other_het = (prepost_reg_joined ==2) & het==1;
gen rdpother_het = rdp_het*other_het;

gen rdp = placebo==0 & het==0; 
gen post = (prepost_reg_joined ==1 ) & het==0; 
gen rdppost = rdp*post; 
gen other = (prepost_reg_joined ==2) & het==0;
gen rdpother = rdp*other;

global dists_all "rdp_het post_het rdppost_het other_het rdpother_het
                   rdp post rdppost other rdpother ${dists_all}";
...
preserve;
  ...
  * grab continuous var from varname;
  destring parm, gen(contin) i(post_ rdp het dists long_and_far .) force;
  ...
  *reaarrange continuous var;
  ...
  tw
    (rspike max90 min90 contin if het==1, lc(gs3) lw(thin) )
    (connected estimate contin if het==1, msymbol(diamond) msiz(medium) mlc(gs0) mfc(gs0) lc(gs0) lp(none) lw(medium))
    (rspike max90 min90 contin if het==0, lc(gs7) lw(thin) lp(dash) )
    (connected estimate contin if het==0, msymbol(circle) msiz(medium) mlc(gs5) mfc(gs5) lc(gs5) lp(dot) lw(medium))    
    ,
    ...
    legend(order(2 "DDD under 32km from CBD" 4 "DDD over 32km from CBD" ) symx(6) col(1)
...
omit dists_all 
  dists_all_800_het dists_rdp_800_het
  dists_post_800_het dists_rdp_post_800_het
  dists_other_800_het dists_rdp_other_800_het
...
estout  using "price_regDDD_het${V}.tex", replace
  style(tex)
  keep(
    dists_rdp_post_600_het 
    dists_rdp_post_400_het 
    dists_rdp_post_200_het
    dists_rdp_post_600 
    dists_rdp_post_400 
    dists_rdp_post_200
  )
  varlabels(
    dists_rdp_post_600_het "Near CBD 400m to 600m" 
    dists_rdp_post_400_het "Near CBD 200m to 400m" 
    dists_rdp_post_200_het "Near CBD 0 to 200m"
    dists_rdp_post_600 "Far from CBD 400m to 600m" 
    dists_rdp_post_400 "Far from CBD 200m to 400m" 
    dists_rdp_post_200 "Far from CBD 0 to 200m"
    ,
    el(
      dists_rdp_post_200_het [0.5em] 
      dists_rdp_post_400_het [0.5em] 
      dists_rdp_post_600_het [0.5em] 
...
preserve;
  ...
  *sort contin;
  ...
  graphexportpdf DDDplot_pertime_alt_unspaghetti_het, dropeps;
