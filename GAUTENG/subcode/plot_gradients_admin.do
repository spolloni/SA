...
global treat = 300;   /* distance to be considered treated  */

global treat_no = 3;  /* number of treatments */
...
global ifregs = "
       ...
       purch_yr > 2000 & count <= `=${treat_no}'
       ";

* distance regression;
*reg lprice b$max.dists#b0.post i.purch_yr#i.purch_mo i.cluster erf*  if $ifregs, cl(cluster);
*plotreg distplot distplot_admin;

* pause on;
* pause;

* time regression;
reg lprice b1001.mo2con_reg#b0.treat i.purch_yr#i.purch_mo i.cluster if $ifregs, cl(cluster);
* plotreg timeplot timeplot_admin;
  ...
  preserve;
   ...
   keep if group==1;
      ...
      *replace contin = contin + .25 if group==1;
      ...
      tw 
      (rcap max95 min95 contin, lc(gs0) lw(thin) )
      (connected estimate contin, ms(o) msiz(small) mlc(gs0) mfc(gs0) lc(gs0) lp(none) lw(thin)),
