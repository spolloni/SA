...
prog def welfare
  ...
  g cuts = .
  forvalues r = 1/10 {
    global rlow =E[1, `r'+$cut - 1]
    global rhigh=E[1, `r'+$cut ] 
    ...
    * global lr = `r'*E[1,$rlow] - $lr
    global lr = ($rlow - (`r'-1)*$lr)/`r'
    }
    disp `r'
    disp $lr 
    disp $rlow
    replace cuts = $lr in `r'
  }
  ...
  forvalues r = 1/10 {
    ...
    if `r'==10 {
    replace k = `r' if  u1>= $rlow
    replace ka = `r' if  u1a>= $rlow
    }
    else {
    replace k = `r' if  u1>= $rlow & u1<= $rhigh
    replace ka = `r' if  u1a>= $rlow & u1a<= $rhigh
    }
    *tab k
