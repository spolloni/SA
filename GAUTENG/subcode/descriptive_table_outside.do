...
local qry = "
    ...
    *drop if cluster_rdp==. & cluster_placebo==.;
    ...
    g rdp=2;
    replace rdp = 1 if cluster_rdp!=.;
    replace rdp = 0 if cluster_placebo!=.;

    g rdp1 =0 if rdp==0;
    replace rdp1=1 if rdp==1;

    replace cbd_dist=. if rdp==2;
...
*g rdp = project_rdp==1;
    ...
    g rdp=2;
    replace rdp = 1 if project_rdp==1;
    replace rdp = 0 if project_placebo==1;
...
if $price_prep == 1 {;
	...
	*	g rdp = distance_rdp<0;
    ...
    g rdp=2;
    replace rdp = 1 if distance_rdp<0;
    replace rdp = 0 if distance_placebo<0;
...
prog define table_prepping;
	...
	estpost sum $varlist if rdp==1;
		matrix c2=e(mean);
	estpost sum $varlist if rdp==2;
		matrix c3=e(mean);
	estpost sum $varlist_ttest;
		matrix c4=e(mean);
		
		matrix A1 = (c1',c2',c3',c4');

	g colnames = "";
	replace colnames = "T-Stat:" in 4;
	replace colnames = "Unconstructed" in 5;
	replace colnames = "Constructed" in 6;
	replace colnames = "Within 4 km." in 7; 
	replace colnames = "Cons. = Uncons." in 8; 
...
prog define table_prepping_no_t;
	...
	replace colnames = "Within 4 km." in 3; 
end;
	...
	*	replace temp = "Population (per km2)" 				in 6;
		...
		table_prepping;
			...
			tables pre_descriptives_outside A1 FOR temp colnames 1;
	...
	*	replace temp = "Population (per km2)" 				in 6;
		...
		table_prepping_no_t;
			...
			tables pre_descriptives_no_t_outside A1 FOR temp colnames 0;
	...
	preserve;
		...
		*replace pop_density = pop_density*100;
		...
		table_prepping;
			...
			tables pre_descriptives_census_outside A1 FOR temp colnames 1 ;
	restore;
