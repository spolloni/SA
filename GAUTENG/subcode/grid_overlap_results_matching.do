...
preserve
  ...
  * keep cluster_joined RDP bfor binf bpop_density
  ...
  keep if binf>=.5 & binf<=25

  keep cluster_joined
  save "matching.dta", replace
...
preserve
  ...
  hist bfor, by(RDP)
  hist bfor if bfor>=.5 & bfor<=18, by(RDP)
  keep  if bfor>=.5 & bfor<=18
  * keep cluster_joined RDP bfor binf bpop_density
  gegen cjtag = tag(cluster_joined)
  keep if cjtag==1

  keep cluster_joined
  save "matching2.dta", replace
...
preserve 
merge m:1 cluster_joined using "matching3.dta", keep(3) nogen
...
global dist     = 0
rfull inf_census_test_matching3
...
preserve 
merge m:1 cluster_joined using "matching2.dta", keep(3) nogen
...
global dist     = 0
rfull inf_census_test_matching2
...
merge m:1 cluster_joined using "matching.dta", keep(3) nogen
...
global dist     = 0
rfull inf_census_test_matching
