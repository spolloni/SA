...
FROM 
(SELECT A.* FROM dropplacebo_grid_temp_100_4000_buffer_area_int_${dist_break_reg1}_${dist_break_reg2} AS A 
LEFT JOIN gcro_over_list AS G ON G.OGC_FID = A.cluster 
WHERE G.dp IS NULL) AS A
JOIN dropplacebo_buffer_area_${dist_break_reg1}_${dist_break_reg2} AS B ON A.grid_id = B.grid_id ";
odbc query "gauteng";
odbc load, exec("`qry'") clear; 
...
save "dropplacebo_buffer_grid_${dist_break_reg1}_${dist_break_reg2}_overlap.dta", replace;
...
ren id grid_id ;
fmerge m:1 grid_id using "dropplacebo_buffer_grid_${dist_break_reg1}_${dist_break_reg2}_overlap.dta" ;
drop if _merge==2;
drop _merge;
ren grid_id id;

save dropplacebo_bbluplot_grid_${grid}_${dist_break_reg1}_${dist_break_reg2}_overlap, replace;
