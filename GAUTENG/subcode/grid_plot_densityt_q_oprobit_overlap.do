...
fmerge m:1 id using "grid_elevation_100.dta"
drop if _merge==2
drop _merge
...
write "con_proj_count_overlap.tex" $con_num .1 "%12.0fc"
write "con_proj_count_overlap.csv" $con_num .1 "%12.0g"

write "plot_per_proj_overlap.tex" $plot_per_proj .1 "%12.0fc"
write "plot_per_proj_overlap.csv" $plot_per_proj .1 "%12.1g"

write "plot_per_spill_overlap.tex" $plot_per_spill .1 "%12.0fc"
write "plot_per_spill_overlap.csv" $plot_per_spill .1 "%12.1g"
...
g PR_post_conPR = PR_post*conPR
  ...
  * keep $regressors for inf inf_backyard inf_non_backyard total_buildings cluster_joined CA garea hmean_f
  cap drop for_cut
  g for_cut = round(for*25*25/($grid*$grid),1)
  cap drop inf_cut
  g inf_cut = round(inf*25*25/($grid*$grid),1)

  replace for_cut = $cutbuild if for_cut>$cutbuild
  replace inf_cut = $cutbuild if inf_cut>$cutbuild
...
preserve
  sample 10
  cmp ( for_cut =  post PR PR_conPR PR_post PR_post_conPR ) ( inf_cut =  post PR PR_conPR PR_post PR_post_conPR ), indicators(5 5) nolrtest cluster(cluster_joined) robust
restore
