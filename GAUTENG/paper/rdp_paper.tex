...
One concern would be that the project construction may be driven by changes in local political or economic conditions that may also drive changes in local housing development.  For example, growing neighborhoods may generate greater political resistance to housing projects, delaying project construction despite improvements in local housing.  Alternatively, worsening infrastructure quality in declining neighborhoods may both increase the costs of finishing projects while decreasing local housing conditions.  To the extent that these trends are reflected in average levels of local development, we can evaluate potential sources of endogeneity by comparing baseline characteristics across project areas as in Section~\ref{section:descriptives}.  However, we are unable to test for these factors if changes in local economic and political conditions happen at the same time as project construction.

Another potential concern is that forward-looking households in unconstructed areas may anticipate the eventual construction (or conversion) of these areas and therefore alter their housing investment decisions.  Knowing that these areas may be soon redeveloped, households may choose to construct temporary, informal structures instead of investing in long-term, formal houses.   

Specific versus less specific... ?


Given these potential limitations to comparing unconstructed and constructed areas, we also consider a complementary approach that compares nearby areas to areas just further away from project boundaries.  Under the assumption that areas just nearby projects would have evolved in the same way as areas just further away in the absence of the housing project.  
...
Anticipation (unconstructed knowing something will pop-up)
...
\multicolumn{3}{l}{\scriptsize $^\dagger$ One USD averaged to about 7.70 Rands during the 2001-2011 period.}\\[-.5em]
...
To contextualize our main analysis, we first provide descriptive evidence on the location, size, and surrounding housing stock of our 133 sampled housing projects. Table~\ref{table:projectdescriptives} reveals that constructed projects provide generous quantities of housing, delivering on average more than 850 new units per project. Constructed projects are also substantially larger in area than unconstructed projects. As mentionned in section \ref{section:data} , this is due to many unconstructed projects representing smaller planned extensions to previously implemented projects. \iffalse It may also reflect that larger land plots are first targeted by housing authorities to maximize delivery.\fi Though unconstructed projects are located in slighty more remote locations, both types of projects impose long commutes to residents, as measured by the average distance to central business districts. This is consistent with housing authorities targeting inexpensive, vacant land for these projects. Finally, prices for non-subsidized housing in the vicinity of project boundaries are slightly lower for unconstructed projects, perhaps in part driven by their more distant locations.

We next turn to household census data to assess how both types of projects differ in terms of dwelling characteristics prior to project construction. We identify census small areas as pertaining to a housing project when over 30\% of their land areas overlaps with a project boundary.\footnote{Our census geography selection method is depicted in more detail in figure \ref{figure:bufferdesigncensus}.} This results in 1,207 small areas containing a total of 258,502 households. Table \ref{table:projectdescriptivescensus} compares various indicators for these households on the basis of whether they live within the boundaries of a constructed (first column) or planned but unconstructed project (second column). As a reference, the third column shows means for every variable using the entire sample of Gauteng households. 
...
A clear takeaway from table \ref{table:projectdescriptivescensus} is that dwelling quality at baseline for both constructed and unconstructed project areas is far worse than the average Gauteng dwel\-ling. Given the land plots targeted for this policy, we expect project areas to lag behind other areas, and reflect more precarious housing conditions. We find that access to sanitation, water and electricity are between 25 and 50\% lower in both types of project areas relative to the province as a whole. Comparing constructed to unconstructed projects, we find no systematic differences in dwelling quality. While constructed areas are more likely to have flush toilets, electricity for lighting, and a larger amount of rooms, household in unconstructed areas report having better access to piped water and electricity for eating and cooking. We note that a simple t-test rejects the null of means equality for every tabulated variable.

We proceed to examine land-use patterns using the building-based data described in section \ref{section:data:bblu}. Using the constructed 50m$\times$50m grid-cell aggregates, figures \ref{fig:prefor} and \ref{fig:preinf}

Taken together, the findings in this section suggest both that (1) governments select low-cost land to implement projects and (2) the decision to finish project construction does not appear to be strongly correlated with pre-existing levels of development in the project areas.  
...
\begin{figure*}[t!]
        ...
        \end{subfigure}
        \hfill 
        \begin{minipage}[c][0cm][c]{0.5\textwidth}   
        \vspace{-6cm}
        \caption[]
