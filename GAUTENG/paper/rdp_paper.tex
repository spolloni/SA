...
A clear takeaway from table \ref{table:projectdescriptivescensus} is that dwelling quality at baseline for both constructed and unconstructed project areas is far worse than the average Gauteng dwel\-ling. Given the land plots targeted for this policy, we expect project areas to lag behind other areas, and reflect more precarious housing conditions. We find that access to sanitation, water and electricity are between 25 and 50\% lower in both types of project areas relative to the province as a whole. Comparing constructed to unconstructed projects, we find no systematic differences in dwelling quality. While constructed areas are more likely to have flush toilets, electricity for lighting, and a larger amount of rooms, households in unconstructed areas report having better access to piped water and electricity for eating and cooking. We note that a simple t-test rejects the null of means equality for every tabulated variable.

We proceed to examine land-use patterns using the building-based 50m$\times$50m grid-cell aggregates described in section \ref{section:data:bblu}. Figures \ref{fig:prefor} and \ref{fig:preinf} respectively plot formal and informal housing density in the pre-period against distance to project boundary.
