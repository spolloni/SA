...
We locate projects using a map of housing projects obtained from the Gauteng City Regional Observatory, a research unit composed of the Gauteng Provincial Government and two Johanesburg universities.\footnote{\href{url}{www.gcro.ac.za/}}  The map describes \input{figures/total_gcro}housing projects as of 2008, including notes on their completion status.  We classify 166 projects with descriptions including ``current,'' ``under implementation,'' or ``complete'' as constructed projects.  We then classify 140 projects with descriptions including ``proposed,'' ``under planning,'' ``future,'' ``investigating,'' and ``uncertain'' as planned but unconstructed projects.  The remaining 336 projects either do not include descriptions, their descriptions are difficult to classify as either constructed or unconstructed, or their footprints fall within another project.  Appendix Table~\ref{table:projectdescriptions} includes tabulations of the full list of descriptions.  To supplement our understanding of project implementation, we conducted qualitative fieldwork over 7 months between 2015 and 2018, including interviews with housing officials in the municipal government of Johannesburg and provincial government of Gauteng. 
...
Housing projects are located on less developed land plots (``inside'') within more developed neighborhoods (``0 - 1 km'') compared to areas further from housing projects (``2 - 4 km'') across measures of housing quality, public services, and infrastructure access.  These findings match qualitative evidence that policymakers prioritize locating projects on low-cost land nearby employment opportunities \citep{beninterview}.
...
Areas nearby projects (0 - 4 km) are more similar between constructed and unconstructed projects than areas within projects.  This finding may be partially driven by the fact that constructed and unconstructed projects are often clustered nearby each other as indicated in Figure~\ref{figure:map}.
...
\begin{table}[h!]
\centering
\caption{Average Outcomes before Project Construction for Plots Inside and Nearby Projects}\label{table:projectdescriptives}
\vspace{0mm}
...
\toprule
\\[-.5em]
Plot location&\multicolumn{3}{c}{Constructed Project Areas} &\multicolumn{3}{c}{Unconstructed Project Areas}  \\[.5em] 
& Inside & 0 - 1 km  & 2 - 4 km & Inside & 0 - 1 km  & 2 - 4 km \\[.3em]
\\[-.5em]
Buildings (per $\text{km}^{2}$) \\ \midrule
\\[-.5em]
...
\end{tabular}
\begin{tablenotes}
\item \footnotesize The USD averaged around 7.70 Rands during the 2001-2012 period.  
\end{tablenotes}
\end{threeparttable}
}
\end{table} 
...
\midrule
\\[-.5em]
Constructed project overlap with  \\ 
\\[-.7em]
\input{figures/proj_rdp_exp}
\\[-.5em]
\hspace{2em}Rings around plots (in km)   \\
\input{figures/spill_R}
\\
Unconstructed project overlap with \\ 
\\[-.7em]
\input{figures/proj_placebo_exp}
\\[-.5em]
\hspace{2em}Rings around plots (in km)   \\
\input{figures/spill_P}
\bottomrule
\end{tabular}
\begin{tablenotes}
\item \footnotesize Sample includes plots within 4 km of projects.  ``Project overlap with plots'' measures the percentage of each plot's area that overlaps with projects.  ``Project overlap with rings around plots'' measures the percentage of the area of rings around each plot that overlap with projects and is only measured for plots outside of projects.
\end{tablenotes}
\end{threeparttable}
% }
\end{table}
...
For plots that overlap with project footprints, we measure direct exposure in terms of the percentage of a plot's area that overlaps with project footprints.  Figure~\ref{fig:insideproj} includes an example plot that overlaps with a constructed project (upper) as well as an example plot that overlaps with an unconstructed project (lower).  Since the upper plot only partially overlaps with a constructed project, its exposure lies between 0 and 1.  The lower plot falls completely within an unconstructed project indicating exposure equal to 1.  On average across all plots, overlap between plots and projects equals 4.6\% for constructed projects and 3.8\% for unconstructed projects. Table~\ref{table:spatialsummary} summarizes this measure.

For plots that do not overlap with project footprints, we measure spillover exposure by first constructing .5 km rings around each plot and second calculating the percentage area of each ring that overlaps with project footprints.  Figure~\ref{fig:outsideproj} provides an example plot that falls just outside of project footprints but whose neighborhood rings from 0 to .5 km and from .5 to 1 km overlap with both constructed and unconstructed projects.  Table~\ref{table:spatialsummary} summarizes these measures across neighborhood rings and constructed and unconstructed projects.  We find that exposure increases with the radius of rings growing from 0.9\% for 0 to .5 km rings up to 3.7\% for 3.5 to 4 km rings for constructed projects.  
...
\label{eq:main}
\begin{split}
\quad y_{it}  =\,&\mathbbm{1}\Big\{\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)>0\Big \}\,\times \Bigg[ \, ( \alpha_0 \, +  \, \alpha_1 \textsc{\small Post}_{t}) \, \times \, \frac{\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)}{\text{Area}\big(\textsc{Plot}_i\big)}  \, + \\[.5em]
&\hspace{14.5em}( \alpha_2 \, +  \, \alpha_3 \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Plot}_i \cap \textsc{Con. Proj.}\big)}{\text{Area}\big(\textsc{Plot}_i\big)}\, \Bigg] \, + \\[.7em]
&\mathbbm{1}\Big\{\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)=0\Big \}\,\times \Bigg[ \, \sum_{k=1}^{K} \, (\beta_{0}^{k} \,+\, \beta_{1}^{k} \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Ring}_i^{k}  \cap  \textsc{Proj.}\big)}{\text{Area}\big(\textsc{Ring}_i^{k}\big)} \, + \\[.5em]
&\hspace{16.2em}(\beta_{2}^{k} \,+\, \beta_{3}^{k} \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Ring}_i^{k}  \cap \textsc{Con. Proj.}\big)}{\text{Area}\big(\textsc{Ring}_i^{k}\big)} \, \Bigg]  +  \\[.5em]
&\gamma \, \textsc{Post}_t + \varepsilon_{it}
\end{split}
\end{align}

\noindent $y_{it}$ is the outcome for plot $i$ observed at time $t$.  $\textsc{Post}_{t}$ equals one after scheduled construction and zero otherwise.  $\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)$ measures the area of overlap between plot $i$ and all project footprints while $\text{Area}\big(\textsc{Plot}_i \cap \textsc{Con. Proj.}\big)$ measures the area of overlap between plot $i$ and only project footprints that were successfully constructed.  Likewise, $\text{Area}\big(\textsc{Ring}_i^{k}  \cap  \textsc{Proj.}\big)$ measures the area of overlap between a ring from $.5(k-1)$ to $.5k$ km around plot $i$ and all project footprints.   $\text{Area}\big(\textsc{Ring}_i^{k}  \cap  \textsc{Proj. Con.}\big)$ measures the area of overlap for this ring and only project footprints that were successfully constructed.  We consider concentric 0.5 km rings ranging from 0 to 4 km away from plots, which implicitly assumes that any spillover effects dissipate by 4 km consistent with recent literature on place-based policies.\footnote{\cite{diamond2016wants} and \cite{rossi2010housing} find that main effects are concentrated within 1 km of place-based policies.}  Figure~\ref{fig:spatialexposure} includes an example of rings from 0 to 0.5 km and 0.5 to 1 km.  $\varepsilon_{ipt}$ is the idiosyncratic error term.  Standard errors are clustered at the project-level with projects assigned to each plot with the greatest area of overlap either with the plot's footprint or with the closest overlapping ring.  

For plots that overlap with project footprints (ie. $\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)>0$), we estimate the direct effects of project construction.  Our coefficient of interest, $\alpha_3$, measures the change in outcomes for plots overlapping with constructed projects relative to plots overlapping with planned but unconstructed projects (weighted by the percentage of a plot's area that overlaps with projects).  Likewise, for plots outside of project footprints (ie. $\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)>0$), we estimate the spillover effects of project construction.  Each coefficient of interest, $\beta_3^{k}$, measures the differential change in outcomes for plots whose neighborhoods from $.5(k-1)$ to $.5k$ km overlap with constructed project footprints relative to plots whose neighborhoods overlap with unconstructed project footprints (weighted by the percentage of a plot neighborhood's area that overlaps with projects).  These coefficients capture the causal effects of project construction under the assumption that in the absence of project construction, plots exposed to constructed project footprints would have evolved in the same way as plots exposed to unconstructed project footprints.\footnote{We do not estimate spillover effects for plots within project footprints because spillover effects for these plots would be mainly identified from the relative locations of plots within project footprints (ie. near the edge versus in the interior of a footprint) and these relative locations within plots are likely to capture variation in where project houses are constructed within footprints instead of the true spillover effects of having greater nearby exposure to projects.}
...
We first investigate how housing projects impact housing and population density within and around their footprints by estimating equation (\ref{eq:main}).  

To examine how project neighborhood effects extend across space, Figure~\ref{fig:gr_coef} graphs coefficients for constructed project overlap with .5 km neighborhood rings from 0 to 4 km ($\beta_3^{1}$ to $\beta_3^{8}$) for two outcomes: people per $\text{km}^{2}$ and houses per $\text{km}^{2}$.  Coefficients are statistically significant at the 1\% level and positive for overlap with 0-.5 km rings.  This finding suggests that exposure to housing projects in immediate neighborhoods invites large increases in plot-level population and housing densities.  Coefficients for rings beyond .5 km are statistically insignificant and hover around zero, which indicates that neighborhood effects quickly dissipate at greater distances.  Zero effects for exposure further from projects further suggests that constructed and unconstructed projects are unlikely to experience differential policies or shifts in economic growth that affect areas evenly within 4 km of projects, supporting our identification assumption of parallel trends in outcomes between constructed and unconstructed projects.  In the following tables, we report results for the closest ring (0 to .5 km) while including estimates for all rings in the Appendix.

Table~\ref{table:main} includes coefficients for the effect of overlapping with constructed project areas ($\alpha_3$) for plots within projects and the effect of a 0 to .5 km neighborhood overlapping with constructed projects ($\beta_3^{1}$) for plots outside of projects.  The first row of column (1) indicates that moving from 0 to 100\% plot overlap with constructed projects generates an increase in 2,574 people per $\text{km}^{2}$.  Likewise, for plots that do not overlap with projects, the second row indicates that moving from 0 to 100\% overlap of a 0 to .5 km neighborhood with constructed projects generates an increase of 2,103 people per $\text{km}^{2}$.  We calculate the total effect per project by multiplying the coefficients by the total plot footprint and neighborhood exposures per project.  We calculate total footprint exposure per project equal to 1.19, which is also the average size of projects in $\text{km}^{2}$, and total neighborhood exposure per project equal to 0.24.  With this formula, coefficients from Table~\ref{table:main} indicate that projects lead to increases of 3,083 people within their footprints and 512 people in their immediate neighborhoods.  


% and 1,091 houses 
% and 693 houses 

On average per project, the total effect in plot footprints equals the coefficin

plot footprint exposure equals 1.19 $\text{km}^{2}$ while neighborhood effects 




On average, projects cover 1.19 $\text{km}^{2}$ of area and their neighborhood effects from 0 to .5 km extend to .24 $\text{km}^{2}$ on average.

% We estimate equation (\ref{eq:main}) 
...
\small
\centering
\caption{Estimates: Population and Housing}\label{table:main}
\vspace{-2mm}
\resizebox{1\linewidth}{!}{
\begin{threeparttable}
...
\end{tabular}
\begin{tablenotes}
\item \footnotesize Population density is from census data.  Houses are from the building data. \regtext
...
\begin{figure*}[hbtp]
    ...
    \begin{subfigure}[b]{.49\textwidth}
        \centering
        \caption[]{\small People per $\text{km}^{2}$}  
        \vspace{-1mm}
    ...
    \begin{subfigure}[b]{.49\textwidth}  
        \centering 
        \caption[]{\small Houses per $\text{km}^{2}$}
        \vspace{-1mm}
...
\small
\centering
\caption{Effects per Constructed Project: Population and Housing}\label{table:main}
\vspace{-2mm}
\resizebox{1\linewidth}{!}{
\begin{threeparttable}
\begin{tabular}{lCCCCC}
\toprule
& (1)&(2)&(3)&(4)&(5)\\[.5em] 
&People & Houses & Formal houses & Informal houses & Informal backyard houses \\\midrule \\[-.8em]
\input{figures/main_new_e}
\bottomrule
\end{tabular}
\begin{tablenotes}
\item \footnotesize \ballparktext 
\end{tablenotes}
\end{threeparttable}
}
\end{table}
...
\small
\centering
\caption{Estimates: Public Services per Household}\label{table:inf_census}
\vspace{-2mm}
\resizebox{1\linewidth}{!}{
\begin{threeparttable}
\begin{tabular}{lCCCCC}
...
\end{tabular}
\begin{tablenotes}
\item \footnotesize Outcomes are from census data. \regtext
\end{tablenotes}
\end{threeparttable}
}
\end{table}
...
\small
\centering
\caption{Estimates: Public Services Buildings}\label{table:inf_census}
\vspace{-2mm}
\resizebox{1\linewidth}{!}{
\begin{threeparttable}
...
\small
\centering
\caption{Effects per Constructed Project: Public Services Buildings}\label{table:main}
\vspace{-2mm}
% \resizebox{1\linewidth}{!}{
\begin{threeparttable}
\begin{tabular}{lCCCC}
\toprule
& (1)&(2)&(3)&(4)\\[.5em] 
&Water Utility Buildings & Electricity Utility Buildings & Health Centers & Schools \\\midrule \\[-.8em]
\input{figures/inf_bblu_e}
\bottomrule
\end{tabular}
\begin{tablenotes}
\item \footnotesize  \ballparktext 
...
\small
\centering
\caption{Estimates: Agglomeration Outcomes}\label{table:inf_census}
\vspace{-2mm}
\resizebox{1\linewidth}{!}{
\begin{threeparttable}
\begin{tabular}{lCCCC}
...
\end{tabular}
\begin{tablenotes}
\item \footnotesize Businesses are from the building data.  Household income and employment are from the census data. \regtext
\end{tablenotes}
\end{threeparttable}
}
\end{table}
...
\toprule
 & \small (1) & \small (2)  & \small (3) & \small (4) & \small (5)   \\
 & \small Has Piped Water Indoors & \small Has a Flush Toilet &  \small Has Electricity & \small Total Rooms & \small People per $\text{km}^{2}$ \\ \midrule
\input{figures/census_overlap}\bottomrule
\end{tabular}
\begin{tablenotes}
\item Standard errors clustered at the project level in parenthesis.\textsuperscript{c} p$<$0.10,\textsuperscript{b} p$<$0.05,\textsuperscript{a} p$<$0.01
Outcomes in Columns (1), (2), and (3) are indicator variables.
...
\toprule
 & \small (1) & \small (2)  & \small (3) & \small (4) & \small (5)   \\
 & \small Has Piped Water Indoors & \small Has a Flush Toilet &  \small Has Electricity & \small Total Rooms & \small People per $\text{km}^{2}$ \\ \midrule
\input{figures/census_spill}\bottomrule
\end{tabular}
\begin{tablenotes}
\item Standard errors clustered at the project level in parenthesis.\textsuperscript{c} p$<$0.10,\textsuperscript{b} p$<$0.05,\textsuperscript{a} p$<$0.01
Outcomes in Columns (1), (2), and (3) are indicator variables.
...
\toprule
 & \small (1) & \small (2)  & \small (3) & \small (4) & \small (5)   \\
 & \small Has Piped Water Indoors & \small Has a Flush Toilet &  \small Has Electricity & \small Total Rooms & \small People per $\text{km}^{2}$ \\ \midrule
\input{figures/census_3}\bottomrule
\end{tabular}
\begin{tablenotes}
\item Standard errors clustered at the project level in parenthesis.\textsuperscript{c} p$<$0.10,\textsuperscript{b} p$<$0.05,\textsuperscript{a} p$<$0.01
Outcomes in Columns (1), (2), and (3) are indicator variables.
