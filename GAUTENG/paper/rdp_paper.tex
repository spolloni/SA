...
\multicolumn{3}{l}{\scriptsize $^\dagger$ The USD averaged around 7.70 Rands between 2001 and 2011.}\\[-.5em]
...
\input{figures/census_hh_DDDregs}
\bottomrule
...
\begin{figure*}[t!]
		\caption{Building Density Estimation}
        \label{fig:DDDplots}
        \centering
        \begin{subfigure}[b]{0.49\textwidth}
            ...
            \vspace{-2mm}
            \includegraphics[width=\textwidth,trim={.5cm .8cm .3cm 0cm}, clip=true]{figures/distplotDDD_bblu_total_buildings_admin}
            \label{fig:DDDtotal}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.49\textwidth}  
            ...
            \vspace{-2mm}
            \includegraphics[width=\textwidth,trim={.5cm .8cm .3cm 0cm}, clip=true]{figures/distplotDDD_bblu_for_admin}     
            \label{fig:DDDformal}
        \end{subfigure}
        \vskip -1mm \vskip 0pt
        \begin{subfigure}[b]{0.49\textwidth}
            ...
            \vspace{-2mm}
            \includegraphics[width=\textwidth,trim={.5cm .8cm .3cm 0cm}, clip=true]{figures/distplotDDD_bblu_inf_admin.pdf}
        ...
        \begin{subfigure}[b]{0.49\textwidth}  
            ...
            \vspace{-2mm}
            \includegraphics[width=\textwidth,trim={.5cm .8cm .3cm 0cm}, clip=true]{figures/distplotDDD_bblu_inf_backyard_admin}
        ...
        \begin{subfigure}[b]{.49\textwidth}  
            ...
            \vspace{-2mm}
            \includegraphics[width=\textwidth,trim={.5cm .8cm .3cm 0cm}, clip=true]{figures/distplotDDD_bblu_inf_non_backyard_admin}    
...
\small
\centering
\caption{Triple Difference Estimates }\label{table:DDD}
\vspace{-2mm}
...
%Figure~\ref{figure:housecensusexample} in Appendix~\ref{appendix:bblupictures} demonstrates this resolution by depicting all structures within and around an example housing project.  To construct a measure of housing density, we take the average number of houses within a 50 meter by 50 meter grid cell.  Figure~\ref{figure:housedensityexample} in Appendix~\ref{appendix:bblupictures} provides an example of this density measure over a housing project.  [ REFERENCE THIS ABOVE IN THE DESCRIPTIVES ]

The high geographic resolution of our housing density measure allows us to take a non-parametric approach to estimating the impacts of housing projects on local building density.  This geographic resolution is instrumental in detecting possible spillover effects just outside of housing project boundaries.   Using these measures, we estimate the following triple-differences specification:
\begin{align}
\label{equation:bblu}
\begin{split}
y_{ipt} \, &= \, \beta_{0} +  \beta_{1}  C_p  + \beta_{2}^{r} Post_t  + \beta_{3}  C_p Post_t \, \\
& + \sum\limits_{d}   I^d_{i,p} \Big( \beta_{0}^d +  \beta_{1}^d C_p + \beta_{2}^{d}  Post_t + \beta_{3}^{d} C_p Post_t \Big) + \, \varepsilon_{ipt}
\end{split}
\end{align}
$y_{itdp}$ measures the average number of houses for cell $i$ in vicinity of project $p$ observed in year $t$.  $Post_{t}\,\,$=1 if year $t$ is 2011, which references the post period.  $C_{p}\,\,$=1 if project $p$ has been constructed.  Coefficients $\beta_0$ through $\beta_3$ estimate a simple differences-in-differences specification for constructed and unconstructed projects between 2001 and 2011.  We then allow these effects to vary according to distance from the project boundary.  $I^d_{ip}$\,=1 if cell $i$ is at distance $d$ from the boundary of project $p$.  $\beta_{0}^{d}$ through $\beta_{3}^{d}$ capture the differential changes in outcomes for cells at $d$ distance.  We consider 50 meter distance bins ranging from 400 meters within project areas (which we assign negative values) to 1,100 meters outside of project areas.  We designate the maximum distance from boundaries (between 1,100 to 1,200 meters) so that coefficients $\beta_{0}^{d}$ through $\beta_{3}^{d}$ can be interpreted as the changes in outcomes for distance bin $d$ relative to areas between 1,100 and 1,200 meters.  This assignment guarantees
		
Notice $I_{i,p}$

$\beta_0$ through $\beta_3$
...
We consider five housing density measures: 1) total houses, 2) formal houses, and 3) informal houses, which are disaggregated into 4) informal backyard houses and 5) informal non-backyard houses.  



We compute the differential changes in density at a variety of distances from project boundaries.  $I^d_{ip}$\,=1 if cell $i$ is at distance $d$ of project $p$.  


Interpreting the coefficients of interest, $\alpha^d$, as causal effects of project construction requires the assumption that at each distance from a project boundary, changes in housing density for constructed areas would have mirrored the changes for unconstructed areas in the absence of construction.  We designate the maximum distance from boundaries (between 1,100 to 1,200 meters) as the reference category so that all differences-in-differences coefficients $\alpha^d$ can be interpreted relative to this category.

Figure~\ref{fig:DDDplots} provides the results of this estimation graphically for all five density measures.  Total building




informal houses in Figure~\ref{fig:dddinformal}, and aggregating to total houses in Figure~\ref{figure:dddtotal}.  These figures plot difference-in-differences coefficients for each distance threshold relative to housing project boundaries.  Standard errors are clustered at the project-level for the \input{figures/total_cluster_rdp}constructed and \input{figures/total_cluster_placebo}unconstructed areas.  Estimates are especially noisy within project areas because the varying sizes of project footprints mean that observations per distance threshold decline quickly moving further within the footprints.
