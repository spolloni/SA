...
\section{Conceptual Framework}\label{section:theory}

\begin{itemize}
	\item Separate informal and formal housing markets
	\item Housing projects produce 
		\begin{enumerate}
			\item amenity effect
				\begin{itemize}
					\item decrease in formal housing prices in rich areas - negative amenity
					% \item (don't really know about amenities in poor places)
				\end{itemize}
			\item supply effect
				\begin{itemize}
					\item decrease in nearby informal housing supply - crowd-out (slum eradication nearby.. could be positive?)
						\begin{itemize}
							\item also proportional with income quartile
							\item indicates really elastic supply of informal housing
						\end{itemize}
					\item no change for formal housing supply
				\end{itemize}
		\end{enumerate}
\end{itemize}
...
\end{split}
\end{align}
\noindent where $q$ indexes neighborhood income quartiles, and $Q_s^q$ takes a value of one if average household income for neighborhood $n$ falls within quartile $q$.  Average income is measured according to household incomes reported in the 2001 Census of Population and Housing.  
...
& \small (1) & \small (2) & \small (3) & \small (4) \\
 & \small Total Housing & \small Total Formal Housing & \small  Informal Non-Bkyd. Housing  & \small Informal Backyard Housing  \\ \midrule 
...
\multicolumn{5}{l}{\footnotesize Outcomes are measure in terms of houses per $\text{km}^{2}$. }\\[-.3em]
\multicolumn{5}{l}{\footnotesize Estimates control for 2001 census sub-place fixed effects.}\\[-.3em]
\multicolumn{5}{l}{\footnotesize Coefficients are interactions with constructed, $\textsc{Const}_{p}$  and post, $\textsc{Post}_{t}$ indicators.}\\[-.3em]
\multicolumn{5}{l}{\footnotesize Appendix Table~\ref{table:bbluDDDfull} includes all corresponding triple-difference coefficients.} 
\end{tabular}
\end{table}
...
\toprule
& \small (1) & \small (2) & \small (3) & \small (4) \\
 & \small Total Housing & \small Formal Housing & \small Informal Backyard Housing & \small Informal Non-Bkyd. Housing \\ \midrule 
\input{figures/b_ksp_o0_d250_500}
\bottomrule
\multicolumn{5}{l}{\footnotesize Standard errors clustered at the project level in parenthesis. \textsuperscript{c} p$<$0.10,\textsuperscript{b} p$<$0.05,\textsuperscript{a} p$<$0.01 }\\[-.3em]
\multicolumn{5}{l}{\footnotesize Project areas are 25$\times$25m grid cells within 1,500m of project footprints.}\\[-.3em]
\multicolumn{5}{l}{\footnotesize Housing densities are measure in terms of houses per $\text{km}^{2}$.}\\[-.3em]
\multicolumn{5}{l}{\footnotesize ``inside'' means within project footprint.  ``constr'' means constructed.} 
\end{tabular}
\end{table}
...
\toprule
 & \small (1)  & \small (2) & \small (3) & \small (4) \\
 & Total Housing & Formal Housing   &Informal Non-Bkyd. Housing  &  Informal Bkyd. Housing  \\ \midrule
...
% \small
% \centering
% \caption{Census Household-level Estimates in Spillover Area by Neighborhood Income Quartile}\label{table:censusestimateshet}
% \vspace{-2mm}
% \begin{tabular}{lDDDDDD}
% \toprule
%  & \small (1) & \small (2)  & \small (3) & \small (4)  & \small (5) & \small (6) \\
%  & \small Is a Formal House & \small Has a Flush Toilet & \small Has Piped Water Indoors  & \small Has Electricity & \small Total Rooms  & \small Owns House  \\ \midrule
% \input{figures/ch1_ksp_o0_d250_500_bb1_q_top} 
% % \input{figures/census_hh_DDregs_AGG}
% \bottomrule\\[-.6em]
% \multicolumn{7}{l}{\footnotesize Standard errors clustered at the project level in parenthesis.\textsuperscript{c} p$<$0.10,\textsuperscript{b} p$<$0.05,\textsuperscript{a} p$<$0.01} \\[-.3em] 
% \multicolumn{7}{l}{\footnotesize Income quartiles are from average household income in 2001 census subplaces.} \\[-.3em] 
% \multicolumn{7}{l}{\footnotesize Outcomes in Columns (1) to (4), (6), and (9) to (11) are indicator variables.} \\[-.3em] 
% \multicolumn{7}{l}{\footnotesize Specifications include census 2001 subplace fixed effects.} \\[-.3em] 
...
% \small
% \centering
% \caption{Census Household-level Estimates in Spillover Area by Neighborhood Income Quartile Cont.}\label{table:censusestimateshet2}
% \vspace{-2mm}
% \begin{tabular}{lDDDDDD}
% \toprule
%  & \small (7) & \small (8)  & \small (9)  & \small (10) & \small (11)  & \small (12) \\
...
% \small
% \centering
% \caption{Census Household-level Estimates By Housing Type}\label{table:censusestimateshousetype}
% \vspace{-2mm}
...
% \small
% \centering
% \caption{Census Household-level Estimates By Housing Type}\label{table:censusestimateshousetype2}
% \vspace{-2mm}
% % \resizebox{.95\linewidth}{!}{
% \begin{tabular}{lDDDD}
% \toprule
...
% We examine how prices change differentially over time for constructed relative to unconstructed projects across neighborhood income quartiles and at different distances from project boundaries using the following specification:
% \begin{align*}
% \quad \text{Log-Prices}_{ipnt} \, =& \sum_{q=1}^{4} \, Q_{n}^{q} \, \Bigg[ \, \, \beta_1^{q} \, \textsc{\small Post}_{t}\times\textsc{\small Const}_{p} \, + \, \beta_2 \, \textsc{\small Post}_{t} \, + \, \beta_3 \, \textsc{\small Const}_{p}\,  \\
%  &\, + \sum_{d=0}^{1200} \,D_i^{d} \, \Big( \, \alpha_1^{q,d} \, \textsc{\small Post}_{t}\times\textsc{\small Const}_{p} \, + \, \alpha_2^{q,d} \, \textsc{\small Post}_{t} \, + \, \alpha_3^{q,d} \, \textsc{\small Const}_{p}\, +\, \alpha_4^{q,d} \Big) \, \Bigg] \\
% &\, + \, \theta \, X_{ipnt} \, + \, \lambda_n \, + \, \varepsilon_{ipnt} \quad 
% \end{align*}
% \noindent where distance to project boundary, $d$, ranges from 0 to 1,200 meters in intervals of 250 meters.    Coefficient, $\beta_1^{q}$ captures average differential changes in log-prices for constructed and unconstructed projects pre and post (expected) construction located in neighborhood income quartile $q$.  $D_i^{d}$ takes a value of one for properties, $i$, in 250 meter intervals, $d$, from 0 to 1,250 meters.  The coefficients of interest, $\alpha_1^{q,d}$ capture the differential changes in log-prices between constructed and unconstructed projects pre and post (expected) construction located in for each neighborhood income quartile $q$ and for each distance bin $d$ relative to similar changes for properties located in the omitted distance category, which is the furthest distance bin (1,250 to 1,500 meters).  $\alpha_1^{q,d}$ coefficients track how price changes evolve over distance to project boundaries according to neighborhood income quartile.  Additional controls $X_{ipnt}$ include up to a cubic polynomial in the lot size of properties as well as a fixed effects for the year-month of each transaction.  The specification also includes neighborhood fixed effects $\lambda_n$.  Figure~\ref{figure:pricesoverdistance} plots $\alpha_1^{q,d}$ coefficients with respect to distance to the nearest project boundary across income quartiles.
...
&\, + \, \theta \, X_{ipnt} \, + \, \lambda_n \, + \, \varepsilon_{ipnt} \quad  \\
\text{where}& \\
& D_i^{d} = \mathbbm{1} \Big\{ dist_i \geq (250*d) - 250 \,\,\, \& \,\,\, dist_i \leq (250*d) + 250 \Big\}
\end{align*}
...
&\, + \, \theta \, X_{ipnt} \, + \, \lambda_n \, + \, \varepsilon_{ipnt} \quad \\
\text{Where }& \\
&T_1 = \mathbbm{1} \Big\{\, t \leq -48  \,\Big\} \,, \,\,\, T_2 = \mathbbm{1} \Big\{\, t > -48 \,\,\,\&\,\,\, t<=-24 \,\Big\} \,\,, \\
&T_3 = \mathbbm{1} \Big\{\, t > 0 \,\,\,\&\,\,\, t<=24 \,\Big\} \,\, ,\,\,T_4\,=\,\mathbbm{1} \Big\{\, t > 24 \,\,\,\&\,\,\, t<=48 \,\Big\} \, , \,\,\, T_5 = \mathbbm{1} \Big\{\, t \geq 48\,\Big\} 
\end{align*}
...
%             \label{fig:prefor}
%             \centering
%             \includegraphics[width=\textwidth,trim={0.3cm .3cm 0.1cm 0cm}, clip=true]{figures/price_dist_3d_no_ctrl_q_q1.pdf}
%         \end{subfigure}
%         \hfill
%         \begin{subfigure}[b]{0.48\textwidth}
%                     \caption[Network2]%
...
% \begin{table}[h!]
% 	\centering
% 	\caption{Average Census Characteristics in Constructed Areas for Households living in Formal Houses in 2011 by Neighborhood Income Quartile}\label{table:comparisons}
% \vspace{-2mm}
% \begin{tabular}{l*{1}{CCCCC}}
% \toprule
% & Project Areas & \multicolumn{4}{c}{ Non-Project Areas (0-1,500m outside)} \\
% &  & Q1 & Q2 & Q3 &Q4 \\
% \midrule
% \input{figures/census_compare_houses_proj_for_4} 
% \bottomrule
%  \multicolumn{6}{l}{\scriptsize Income quartiles (Q1-Q4) is by average household income in 2001 census sub-places.} \\ [-.5em]
%  \multicolumn{6}{l}{\scriptsize  Averages include only households living in formal houses. } \\ [-.5em]
%  \multicolumn{6}{l}{\scriptsize  Project areas are defined as census blocks with over 30\% overlap with project footprints. } \\ [-.5em]
% % \multicolumn{4}{l}{\scriptsize  area overlap with constructed and unconstructed projects respectively. } \\ [-.5em]
% % \multicolumn{4}{l}{\scriptsize ``All''  includes all small-areas.  Means are weighted by land area.}
% \end{tabular}
% \end{table}
