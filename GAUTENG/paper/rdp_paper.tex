...
\includegraphics[width=.95\textwidth,trim={0cm 0cm 0cm 0cm}]{figures/casestudy5_medium.png}
        \label{fig:bblumaps}
        \small
\end{figure*} 
...
This framework allows for mapping the observed number of houses on each land plot into estimates of amenity values, construction costs, and congestion costs.  Given equation (\ref{eq:housingdemand}) and assuming that amenity shocks, $\epsilon_{hjt}$, are independently, normally distributed, we can express the probability of observing $k$ houses on a given plot of land as an ordered probit model:
\begin{align}
...
% Changes in local net amenities are likely to overstate the full welfare effects of housing projects as renters substitute between project areas, spillover areas, and outside areas.  


To compute the full welfare effects associated with changes in local amenities, we use estimates from Table~\ref{table:housingdemandestimates} to simulate the effect of housing projects on local building growth.  Given simulated choices, we calculate the change in net rents extracted by landlords, which fully summarize total welfare effects.  Appendix~\ref{section:appendixwelfaresim} includes details on the simulation exercise and Table~\ref{table:housingdemandsim} includes predicted net rental changes by housing sector, project and spillover areas, and neighborhood income quartile.
...
\begin{table}[h]
\centering
\caption{Welfare Changes in Rands by Housing Market and Location}\label{table:housingdemandestimates}
\vspace{-2mm}
\begin{threeparttable}
\begin{tabular}{lDD}
\toprule
 &   Formal Housing &  Informal Housing \\ \midrule 
inside project & \input{figures/for_proj_w} & \input{figures/inf_proj_w} \\
0-500m outside & \input{figures/for_spill_w}& \input{figures/inf_spill_w} \\
\bottomrule\\[-.8em]
\end{tabular}
\begin{tablenotes}
