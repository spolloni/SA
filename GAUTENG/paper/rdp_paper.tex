...
% \begin{figure*}[t!]
%         \centering
%         \caption{Pre-Period Log House Prices Outside Constructed and Unconstructed Projects }\label{fig:preprice}
%         % {\small Pre-Period Log House Prices Outside Constructed and Unconstructed Projects  } 
%         %\vspace{2mm}
%         % \begin{subfigure}[b]{0.495\textwidth}
%             % \centering
...
% Figure~\ref{fig:preprice}  performs an analogous exercise with property transaction data from the deeds records.  These records are likely to be composed of formal houses because informal housing transactions are rarely recorded.  Since there are very few non-subsidized housing transactions within project footprints, we focus only on transactions at positive distances.  Using the date of each transaction, we group prices before and after the expected construction dates for both constructed and unconstructed projects.  Figure~\ref{fig:preprice} plots average log purchase prices before the expected construction date for both constructed and unconstructed areas with respect to distance to project footprints.  Moving from 0 to 1,500 meters from project footprints, prices increase by about 0.3 log-points or roughly 30\% for both constructed and unconstructed projects.  Low prices near project boundaries are consistent with reports of public housing officials targeting low-cost land for housing development.
...
\includegraphics[width=.7\textwidth,trim={0cm 0cm 0cm 0cm}]{figures/casestudy4_medium.png}
        \label{fig:bblumaps}
        \small
\end{figure*} 
...
% This triple differences strategy overcomes limitations of two standard differences-in-differences approaches: (1) comparing changes in outcomes nearby housing projects to areas further way from projects and (2) comparing changes in outcomes for constructed projects to planned but unconstructed projects.
...
\\
Correlation of amenity shocks: $\rho_{for,inf}$ &\multicolumn{2}{c}{ $0.620^{\text{a}}$ } \\[-.3em]
 & \multicolumn{2}{c}{ (0.018) } \\[.3em]
...
\multicolumn{3}{l}{ \footnotesize \textsuperscript{c} p$<$0.10,\textsuperscript{b} p$<$0.05,\textsuperscript{a} p$<$0.01 Outcome categories by plot and  } \\[-.3em]
\multicolumn{3}{l}{ \footnotesize  year include categories for formal and informal building counts} \\[-.3em]
\multicolumn{3}{l}{ \footnotesize  from 0 to 9 and a category at least 10.  Table~\ref{table:housingdemandestimates_full} includes  } \\[-.3em]
\multicolumn{3}{l}{ \footnotesize the full set of coefficients.  Net amenities equal local amenities} \\[-.3em]
\multicolumn{3}{l}{ \footnotesize  minus local housing costs.} \\[-.3em]
\end{tabular}
\end{table}
...
% the potential for housing projects to generate local agglomeration economies.  
% Future work may benefit from considering 
...
%  While housing projects remove existing informal houses,

% In the considering spillover effects of place-based policies, looked just outside of project boundaries 



%    Second, while 

% These results stress how the presence of elastic, informal housing markets may impact the welfare effects of place-based policies in developing countries.  We find that housing projects generate large shifts in informal housing supply as well as 





% This setting may also provide broader lessons for implementing place-based policies like housing projects particularly in developing countries.  


% Our evidence suggests that formal housing markets in developing countries may be similar to housing markets in developed countries in responding to nearby place-based policies.  Our price results for formal housing markets parallel \cite{diamond2016wants} who study the impact of low-income housing projects in the US: formal housing prices decline in high income neighborhoods, but shows signs of increasing in low income neighborhoods.  

% Results suggest that informal housing supply may be much more elastic than formal housing supply.  


% Lessons from this setting may also carry over to policies and beyond South Africa, particularly 
% This setting also provides lessons for policymakers beyond South Africa.  We highlight several features 
...
% The South African National According to the National Housing Act of 1997, 
...
% ``use public money available for housing development in a manner which stimulates private investment in, and the contributions of individuals to, housing development''


% ``promote the establishment, development and maintenance of socially and economically viable communities and of safe and healthy living conditions to ensure the elimination and prevention of slums and slum conditions''
...
% 		\end{itemize}
% 	\item Goal 3: Reduce nearby slum growth [quote]
% 		\begin{itemize}
