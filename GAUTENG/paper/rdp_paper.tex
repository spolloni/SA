...
To assess impacts on dwelling quality, we pool the 2001 and 2011 census cross-sections, and test for differential 10-year changes between constructed and unconstructed project areas. Because these data are much coarser spatially, we define two levels of exposure to the policy: $D=\{\text{\small\it project},\text{\small\it spillover}\}$. As in table \ref{table:projectdescriptivescensus}, households labeled as {\it project} live in census small areas with over 30\% area overlap with project boundaries. The remaining {\it spillover} households live in small areas with less than 30\% area overlap, but whose centroids still fall within 1.5 km of a project boundary. Appendix \ref{appendix:bufferdesign} maps an example of {\it project} and {\it spillover} small areas for a given constructed project. With these definitions in hand, we estimate:
...
\noindent where observations within 2001 {\it spillover} small areas for unconstructed projects are the omitted category. We note that in (\ref{eq:censushh}), the $\textsc{\small Post}_{pt}$ dummy simply indicates obervations in the 2011 cross-section. We estimate equation (\ref{eq:censushh}) over the same binary outcomes tabulated in table \ref{table:projectdescriptivescensus}, with the addition of a population density variable computed at the small area level. We construct this additional outcome by computing the sum of household sizes  and dividing it by the square-kilometer area of the census small area.

Table~\ref{table:censusestimates} provides estimates of $\alpha^d$, $\beta^d$, $\gamma^d$, and $\theta$ for each outcome. Rows 1 through 4 describe effects for households in our {\it project} exposure definition, while rows 5 through 7 describe effects for our {\it spillover} definition. Rows 3 and 7 respectively show that for {\it project} and {\it spillover} exposures, the model cannot reject that treated and control project areas are equal at baseline, in 2001. The DID coefficients of interest in the first row (project$\,\times\,$post$\,\times\,$const) document large and statistically significant improvements in access to services such as flush toilets, indoor water taps, and electricity.\footnote{With the exception of electricity for lighting, which falls short of the 10\% significance level. } The economic magnitudes of these estimates are large, more than 10 percentage points, which is consistent with the policy successfully providing higher quality housing.

We do not find corresponding effects for households living in the vicinity of treated areas. Row 5 of table \ref{table:censusestimates} documents that household living in {\it spillover} areas do not experience statistically significant improvement in their living conditions. These results provide little evidence that finishing a housing project differentially benefits housing quality in nearby areas.
...
\begin{figure*}[t!]
    ...
    \vspace{-6cm}
    \caption[]
    {\small DDD coefficients for fives types of housing densities.} \label{fig:DDDbblu}
	\end{minipage}
\end{figure*} 

To assess impacts on housing densities, we pool the 2001 and 2012 grid-cell datasets, and test for differential 10-year changes between constructed and unconstructed project areas. 
...
& \, + \, \alpha^d \textsc{\small Post}_{pt}\times\textsc{\small Const}_{p} \, + \, \beta^d\textsc{\small Post}_{pt} \, + \, \gamma^d\textsc{\small Const}_{p} \, + \, \varepsilon_{ipt} \quad 
\end{aligned}
\end{equation}

Drawing from our setup in figure \ref{fig:rawbblumeans}
