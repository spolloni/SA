...
Plot location& Inside & 0 - 1 km  & 2 - 4 km & Inside & 0 - 1 km  & 2 - 4 km \\[.3em]
\\[-.5em]
...
\begin{tabular}{lCC}
\toprule
& Constructed Overlap (100 $\text{m}^{2}$) & Unconstructed Overlap (100 $\text{m}^{2}$) \\
\midrule
\\[-.5em]
...
\end{tabular}
\begin{tablenotes}
\item \footnotesize Sample includes plots within 4 km of projects.  ``Project overlap with plots'' measures the share of each plot's area that overlaps with projects.  ``Project overlap with rings around plots'' measures the share of the area of rings around each plot that overlap with projects and is only measured for plots outside of projects.
\end{tablenotes}
\end{threeparttable}
% }
\end{table}
...
To examine how spillover effects dissipate with distance, Figure~\ref{fig:gr_coef} graphs coefficients for constructed project overlap with .5 km neighborhood rings from 0 to 4 km ($\beta_3^{1}$ to $\beta_3^{8}$) for two outcomes: people per $\text{km}^{2}$ and houses per $\text{km}^{2}$.  Coefficients are statistically significant at the 1\% level and positive for overlap with 0-.5 km rings.  This finding suggests that exposure to housing projects in immediate neighborhoods drives large increases in a plot's population and housing.  Coefficients for rings beyond .5 km are statistically insignificant and hover around zero, which indicates that spillover effects quickly dissipate at greater distances.  Zero effects for exposure further from projects further suggests that constructed and unconstructed projects are unlikely to experience differential policies or shifts in economic growth that affect all areas evenly within 4 km of projects, providing some support to our identification assumption of parallel trends in outcomes between constructed and unconstructed projects in the absence of construction.  In the following tables, we report results for the closest ring (0 to .5 km) while including estimates for all rings in the Appendix.

Table~\ref{table:main} includes coefficients for the direct effect --- the effect of overlapping with constructed project areas ($\alpha_3$)  --- and the spillover effect --- the effect of a 0 to .5 km ring overlapping with constructed projects ($\beta_3^{1}$) for plots outside of projects.  Column (1) finds a direct effect of 2,574 people per $\text{km}^{2}$ and an indirect effect of 2,103 people per $\text{km}^{2}$.  

On average, each project covers .24

We calculate the total effect per project by multiplying the coefficients by total plot footprint exposure and total 0-.5 km neighborhood exposure per project.  

We calculate total footprint exposure per project equal to 1.19, which is also the average size of projects in $\text{km}^{2}$, and total 0-.5 km neighborhood exposure per project equal to 0.24.  With this formula, coefficients from Table~\ref{table:main} indicate that projects lead to increases of 3,083 people within their footprints and 512 people in their immediate neighborhoods.  
...
\begin{figure*}[hbtp]
    ...
    \begin{subfigure}[b]{.49\textwidth}  
        ...
        \label{fig:gr_house}
    \end{subfigure}
{\footnotesize Confidence intervals are at the 95\% level. }
\end{figure*} 
