...
\input{figures/pre_table_bblu_dist}
% \textbf{Building data mixed} \\[1em]
% \input{figures/pre_table_bblu_mixed}
% \textbf{Building data zeros} \\[1em]
% \input{figures/pre_table_bblu_zeros}
\bottomrule
\end{tabular}
\end{table} 
...
\\[-.5em]
\textbf{Building data} \\[1em]
\input{figures/pre_table_footprint}
% \textbf{Building data mixed} \\[1em]
% \input{figures/pre_table_bblu_mixed}
% \textbf{Building data zeros} \\[1em]
% \input{figures/pre_table_bblu_zeros}
...
\input{figures/pre_table_bblu_spill_shares}
\textbf{Building data DIST} \\[1em]
\input{figures/pre_table_bblu_spill_dist}
\bottomrule
\end{tabular}
}
\end{table} 
...
To estimate project impacts, we compare changes in outcomes between constructed projects and planned but unconstructed projects.  We estimate both the direct effects of overlapping with project footprints as well as the spillover effects of  having projects nearby according to the following equation
...
% \label{eq:main}
\begin{split}
\quad y_{it}  =\, & ( \alpha_0 \, +  \, \alpha_1 \textsc{\small Post}_{t}) \, \times \, \frac{\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)}{\text{Area}\big(\textsc{Plot}_i\big)} \,\, + \\[.5em]
& ( \alpha_2 \, +  \, \alpha_3 \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Plot}_i \cap \textsc{Con. Proj.}\big)}{\text{Area}\big(\textsc{Plot}_i\big)} \,\, + \\[.7em]
& \Bigg[ \, \sum_{k=1}^{K} \, (\beta_{0}^{k} \,+\, \beta_{1}^{k} \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Ring}_i^{k}  \cap  \textsc{Proj.}\big)}{\text{Area}\big(\textsc{Ring}_i^{k}\big)} \, + \\[.5em]
&\hspace{2.2em}(\beta_{2}^{k} \,+\, \beta_{3}^{k} \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Ring}_i^{k}  \cap \textsc{Con. Proj.}\big)}{\text{Area}\big(\textsc{Ring}_i^{k}\big)} \, \Bigg]  \times \mathbbm{1}\Big\{\text{Area}\big(\textsc{Plot}_i  \cap  \textsc{Proj.}\big)=0\Big \}\, +  \\[.5em]
&\hspace{2.2em}\gamma\textsc{Post}_t + \varepsilon_{it}
\end{split}
\end{align}
...
% \begin{split}
% \quad y_{it} \, =\, & ( \alpha_0 \, +  \, \alpha_1 \textsc{\small Post}_{t}) \, \times \, \frac{\text{Area}\big(\textsc{Plot}_i \, \cap \, \textsc{Proj.}\big)}{\text{Area}\big(\textsc{Plot}_i\big)} \,\, + \\[.5em]
% & ( \alpha_2 \, +  \, \alpha_3 \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Plot}_i \,\cap\, \textsc{Con. Proj.}\big)}{\text{Area}\big(\textsc{Plot}_i\big)} \,\, + \\[.5em]
% &\mathbbm{1}\Big\{\text{Area}\big(\textsc{Plot}_i \, \cap \, \textsc{Proj.}\big)=0\Big \}\, \Bigg[ \, \sum_{k=1}^{K} \, (\beta_{0}^{k} \,+\, \beta_{1}^{k} \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Ring}_i^{k} \, \cap \, \textsc{Proj.}\big)}{\text{Area}\big(\textsc{Ring}_i^{k}\big)} \, + \\[.5em]
% &\hspace{2.8em}(\beta_{2}^{k} \,+\, \beta_{3}^{k} \textsc{\small Post}_{t})  \, \times \, \frac{\text{Area}\big(\textsc{Ring}_i^{k} \, \cap \, \textsc{Con. Proj.}\big)}{\text{Area}\big(\textsc{Ring}_i^{k}\big)} \, \Bigg] \, +  \\[.5em]
...
% &\hspace{2.8em}(\beta_{2}^{k} \,+\, \beta_{3}^{k} \textsc{\small Post}_{t})  \, \times \,\textsc{\% Overlap} \,\Big(\textsc{Neighborhood}^{k}_i \,\, , \textsc{ Const. Projects} \Big)  \, \Bigg] \, +  \\[.5em]
% &\gamma\textsc{Post}_t + \varepsilon_{it}
...
\noindent $y_{it}$ is the outcome for plot $i$ observed at time $t$.  $\textsc{\small Post}_{t}$ equals one after scheduled construction and zero otherwise.  $\textsc{\% Overlap} \,\Big(\textsc{Footprint}_i \,\, , \textsc{ Projects} \Big)$ and $\textsc{\% Overlap}$ $\,\Big(\textsc{Footprint}_i$ $, \textsc{Const. Projects} \Big)$ measure the percentage of plot $i$'s footprint that intersects with project footprints and constructed project footprints respectively.  To capture spillover effects for plots that do not intersect with project footprints, we define neighborhoods in terms of 0.5 km rings spanning from 0 to 4 km.  This specification assumes that any spillover effects dissipate by 4 km, which is consistent with recent literature on place-based policies.\footnote{\cite{diamond2016wants} and \cite{rossi2010housing} find that main effects are concentrated within 1 km of place-based policies.}  Figure YY includes an example of neighborhoods from 0 to 0.5 km and 0.5 to 1 km.  $\textsc{\% Overlap}$ $\Big(\textsc{Neighborhood}^{k}_i$ $, \textsc{ Projects} \Big)$  and  $\textsc{\% Overlap}$ $\Big(\textsc{Neighborhood}^{k}_i$ $, \textsc{ Const. Projects} \Big)$ measure the percentage of neighborhood $k$ around plot $i$ that intersects with project footprints and constructed project footprints respectively for plots that do not directly intersect with projects.  $\varepsilon_{ipt}$ is the idiosyncratic error term.  



To estimate the direct effect of housing projects in their footprints, our coefficient of interest, $\alpha_3$, measures the change in outcomes for plots that overlap with constructed projects relative to plots that overlap with unconstructed projects.  This coefficient reflects the causal effect of project construction under the assumption that in the absence of project construction, plots overlapping with constructed project areas would have evolved in the same way as plots overlapping with unconstructed project areas.

We estimate the spillover effects of housing projects on neighboring areas through additional coefficients of interest, $\beta^{k}_3$.  Each $\beta^{k}_3$ measures the differential change in outcomes for plots with neighborhood $k$ overlapping with constructed project areas relative to plots with neighborhood $k$ overlapping with unconstructed project areas  controlling for each plot's overlap in all other neighborhoods.




Our coefficients of interest, $\alpha_3$ and $\beta^{k}_3$, capture differential changes in outcomes for constructed projects versus unconstructed projects depending on the extent to which the footprints and neighborhoods 

.  A causal interpretation requires the assumption that absent housing project construction, changes nearby constructed and unconstructed projects would have evolved similarly relative to changes farther away from projects.  
